...
# 캐릭터 (스프라이트) 불러오기
#character = pygame.image.load("C:\\Users\\ljm03\\Python_Game\\pygame_basic\\character.png")
character = pygame.image.load("C:\\Users\\ljm03\\Python_Game\\pygame_basic\\정민.jpeg")
character_size = character.get_rect().size  # 이미지의 크기를 구해옴 
character_width = character_size[0]  # 캐릭터의 가로 크기 
character_height = character_size[1]  # 캐릭터의 세로 크기
character_x_pos = screen_width / 2 - (character_width / 2)  # 화면 가로의 절반 크기에 해당하는 곳에 위치 
character_y_pos = screen_height - character_height  # 화면 세로 크기에 가장 아래에 해당하는 곳에 위치 
...
# 적 Enemy 캐릭터 
#enemy = pygame.image.load("C:\\Users\\ljm03\\Python_Game\\pygame_basic\\enemy.png")
enemy = pygame.image.load("C:\\Users\\ljm03\\Python_Game\\pygame_basic\\소언.jpeg")
enemy_size = enemy.get_rect().size  # 이미지의 크기를 구해옴 
enemy_width = enemy_size[0]  # 캐릭터의 가로 크기 
enemy_height = enemy_size[1]  # 캐릭터의 세로 크기
enemy_x_pos = (screen_width / 2) - (enemy_width / 2)  # 화면 가로의 절반 크기에 해당하는 곳에 위치 
enemy_y_pos = (screen_height / 2) - (enemy_height / 2)  # 화면 세로 크기에 가장 아래에 해당하는 곳에 위치 
...
# 20 fps : 1초 동안에 20번 동작 -> 1번에 5만큼!  5 * 20 = 100
    ...
    # 타이머 집어 넣기 
    # 경과 시간 계산 
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000   # milliseconds 이기 때문에 1000으로 나눔
