# version_2 : updating score (finished) / adding blocks / adding sound
# Adding blocks per seconds
#
#
##############################################################
import pygame
...
def draw_life(life):
    text_surface_life = font.render(f"Life : {life}", True, WHITE)
    screen.blit(text_surface_life, (10, 10))
    ...
    for column in range(10):
        for row in range(4):
            ...
            if not block_destroyed[column][row] and ball_rect.colliderect(block_rect[column][row]):
                global ball_speed_y, earned_score
                ball_speed_y *= -1
...
def draw_next_block():
    # TO DO : DRAW NEXT BLOCK FALLING FROM THE TOP OF THE WINDOW 
    print("Hello world")
...
def display_score():
    text_surface_score = font.render(f"Score : {earned_score}", True, WHITE)    
    screen.blit(text_surface_score, (350, 10))
